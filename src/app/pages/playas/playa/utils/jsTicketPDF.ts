...
export class jsPDFclient {
  ...
  public generateTicketPDF(item: any, newState: number, playa: any) {
    this._servicioApi.updateStatePlaca(item, newState).subscribe(
      ...
      (error) => {
        this.errorMessage =
          'Error en la Base de Datos, informar del error inmediatamente.';
        this.showAlert = true;
    ...
    doc.text(
      'TICKET PLAYA',
      (doc.internal.pageSize.width - doc.getTextWidth('TICKET PLAYA')) / 2,
      7
    );
    ...
    doc.setFontSize(10);
    doc.text(
      'TARIFA HORA/FRACCION',
      (doc.internal.pageSize.width - doc.getTextWidth('TARIFA HORA/FRACCION')) /
        2,
      13
    );
    ...
    doc.text(
      ...
      (doc.internal.pageSize.width -
        doc.getTextWidth(`${vehiculo}: ${tarifa}`)) /
        2,
      17
    );
    doc.setFont('helvetica', 'bold');
    ...
    doc.text('HORA:', 12, 34);
    doc.text(
      this.datePipe.transform(item.hora_entrada, 'hh:mm a') || 'undefined',
      45,
      34
    );
    doc.text('FECHA:', 12, 39);
    doc.text(
      this.datePipe.transform(item.hora_entrada, 'dd-MM-yyyy') || 'undefined',
      45,
      39
    );
    let padding = 0;
    ...
    doc.text(
      playa.direccion,
      (doc.internal.pageSize.width - doc.getTextWidth(playa.direccion)) / 2,
      70 - 22 + padding
    );
  ...
  public async generatePagoPDF(item: any, newState: number, playa: any) {
    ...
    this.totalHoras = this.calcularHorasEntreFechas(
      item.hora_entrada,
      fechaHora,
      15
    );
    ...
    try {
      if ('id_auto' in item) {
        const response = await this.toPromise(
          this._servicioApi.carroPagoTicketVenta(
            item,
            newState,
            fechaHora,
            this.totalHoras * tarifa
          )
        );
        numBoleta = response.documentId;
      } else {
        const response = await this.toPromise(
          this._servicioApi.motoPagoTicketVenta(
            item,
            newState,
            fechaHora,
            this.totalHoras * tarifa
    ...
    doc.text(
      'TICKET DE VENTA',
      (doc.internal.pageSize.width - doc.getTextWidth('TICKET DE VENTA')) / 2,
      20
    );
    doc.text(
      `OR002-${numBoleta}`,
      (doc.internal.pageSize.width - doc.getTextWidth(`OR002-${numBoleta}`)) /
        2,
      25
    );
    ...
    doc.text(
      `Fecha Entrada: ${
        this.datePipe.transform(item.hora_entrada, 'dd-MM-yyyy hh:mm a') ||
        'undefined'
      }`,
      3,
      40
    ...
    doc.text(
      `Fecha Salida: ${
        this.datePipe.transform(fechaHora, 'dd-MM-yyyy hh:mm a') || 'undefined'
      }`,
      3,
      45
    );
    ...
    doc.text(`${(this.totalHoras * tarifa).toFixed(2)}`, 73, 57.5, {
      align: 'right',
    });
    ...
    doc.text(
      N2W,
      (doc.internal.pageSize.width - doc.getTextWidth(N2W)) / 2,
      64.5
    );
    doc.setFontSize(10);
    doc.text('TOTAL:', 40, 71);
    doc.text(`S./ ${(this.totalHoras * tarifa).toFixed(2)}`, 73, 71, {
      align: 'right',
    });

    doc.text(
      playa.direccion.toUpperCase(),
      (doc.internal.pageSize.width -
        doc.getTextWidth(playa.direccion.toUpperCase())) /
        2,
      78
    );
    doc.text(
      ...
      (doc.internal.pageSize.width -
        doc.getTextWidth('AREQUIPA-AREQUIPA-AREQUIPA')) /
        2,
      82
    );
    doc.setFontSize(12);
    doc.text(
      ...
      (doc.internal.pageSize.width -
        doc.getTextWidth('GRACIAS POR SU PREFERENCIA')) /
        2,
      86
    );
  ...
  public generateTicketVentaPDF(item: any, playa: any): void {
    // Usar datos ya almacenados en la base de datos
    const totalPagar = parseFloat(item.total_pagar) || 0;
    const tarifa = 'id_moto' in item ? playa.tarifaMoto : playa.tarifaAuto;
    const totalHoras = Math.ceil(totalPagar / tarifa) || 1;
    const N2W = this.convertAmountToWords(totalPagar);

    // Obtener número de ticket desde la base de datos con formato profesional
    const ticketNumber = item.ticket_id
      ? item.ticket_id.toString().padStart(8, '0')
      : '00000001';
    const serieTicket = `T001-${ticketNumber}`;
    ...
    doc.text(
      serieTicket,
      (doc.internal.pageSize.width - doc.getTextWidth(serieTicket)) / 2,
      25
    );
    doc.text(`TICKET: ${ticketNumber}`, 3, 30);
    doc.text(`PLACA: ${item.placa}`, 3, 35);
    doc.text(
      `Fecha Entrada: ${
        this.datePipe.transform(item.hora_entrada, 'dd-MM-yyyy hh:mm a') ||
    ...
    // Usar hora_salida si existe, sino usar fecha actual
    const fechaSalida = item.hora_salida
      ? new Date(item.hora_salida)
      : new Date();
    doc.text(
      `Fecha Salida: ${
        this.datePipe.transform(fechaSalida, 'dd-MM-yyyy hh:mm a') ||
        'undefined'
      }`,
      3,
      45
    ...
    doc.text('SERVICIO PLAYA', 18, 57.5);
    doc.text(String(Number(tarifa).toFixed(2)), 56.5, 57.5, { align: 'right' });
    doc.text(`${totalPagar.toFixed(2)}`, 73, 57.5, { align: 'right' });
    doc.line(2, 60.5, 77, 60.5);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(6);
    ...
    doc.setFontSize(10);
    doc.text('TOTAL:', 40, 71);
    doc.text(`S./ ${totalPagar.toFixed(2)}`, 73, 71, { align: 'right' });
  ...
  private convertAmountToWords(amount: number): string {
    ...
    return `SON: ${words.toUpperCase()} CON ${this.getDecimalPart(
      amount
    )}/100 SOLES`;
  }
